...
def seed_demo_stability():
    ...
    cursor.execute(
        "DELETE FROM tickets WHERE owner_id = 'demo_system' AND routing_status = 'escalated'"
    )
    cursor.execute(
        "DELETE FROM pattern_alerts WHERE representative_title LIKE '%Widespread%' OR representative_title LIKE '%Company-wide%'"
    )
    ...
    for alert in alerts:
        ...
        """,
            (
                str(uuid.uuid4()),
                alert["cluster_size"],
                alert["representative_title"],
                alert["category"],
                1,
                json.dumps(alert["ticket_ids"]),
                "active",
                datetime.utcnow().isoformat(),
            ),
        )
