...
async def debug_db():
    ...
    async with engine.connect() as conn:
        try:
            ...
            res = await conn.execute(text("SELECT owner_id, count(*) FROM tickets GROUP BY owner_id"))
            owners = res.all()
            print("\nTickets by Owner:")
            for row in owners:
                print(f"  - {row[0]}: {row[1]}")
            ...
            res = await conn.execute(text("SELECT status, count(*) FROM tickets GROUP BY status"))
            statuses = res.all()
            print("\nTickets by Status:")
            for row in statuses:
                print(f"  - {row[0]}: {row[1]}")
            ...
            res = await conn.execute(text("SELECT count(*) FROM tickets WHERE owner_id = 'kaggle_importer'"))
