...
async def backfill():
    ...
    cursor.execute(
        "SELECT id, title, description, category FROM tickets WHERE owner_id = 'kaggle_importer'"
    )
    ...
    cursor.execute(
        "SELECT id, title, description, category, owner_id, structured_payload FROM tickets WHERE status = 'resolved' LIMIT 500"
    )
    ...
    for i, row in enumerate(rows):
        ...
        cursor.execute(
            "SELECT resolution_steps_json FROM tickets WHERE id = ?", (ticket_id,)
        )
        ...
        try:
            ...
            # Clear existing
            cursor.execute(
                "DELETE FROM similar_tickets WHERE ticket_id = ?", (ticket_id,)
            )
