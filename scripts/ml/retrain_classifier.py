...
def download_datasets_from_minio(output_dir: Path) -> list[Path]:
    ...
    try:
        ...
        client = Minio(
            settings.object_storage_endpoint.replace("http://", "").replace(
                "https://", ""
            ),
...
def map_dataset_fields(record: dict) -> dict:
    ...
    resolution = (
        ...
        or record.get("resolution_notes")
        or record.get("fix")
        or ""
    )
...
def load_and_preprocess(file_paths: list[str]) -> list[dict]:
    ...
    if unmapped_categories:
        print(
            f"\n[WARN] Found {len(unmapped_categories)} unmapped categories:",
            file=sys.stderr,
        )
        for cat in sorted(unmapped_categories):
            print(f"  - '{cat}'", file=sys.stderr)
        print(
            "[INFO] These records will be filtered out. Consider adding mappings to CATEGORY_MAPPING.\n",
            file=sys.stderr,
        )
    ...
    for item in all_data:
        ...
        if "description" in scrubbed_item and scrubbed_item["description"]:
            scrubbed_item["description"], _ = PIIScrubber.scrub(
                scrubbed_item["description"]
            )

        if "resolution" in scrubbed_item and scrubbed_item["resolution"]:
            scrubbed_item["resolution"], _ = PIIScrubber.scrub(
                scrubbed_item["resolution"]
            )
    ...
    print(
        f"[INFO] Filtered out {filtered_count} records with invalid/missing categories"
    )
...
def merge_datasets_with_overrides(
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if isinstance(data, list):
                    ...
                    print(
                        f"[WARN] Skipping non-list dataset: {file_path}",
                        file=sys.stderr,
                    )
        except Exception as e:
    ...
    print(
        f"[INFO] Merged dataset: {len(merged)} samples "
...
async def fine_tune_model(
    ...
    print(
        f"[INFO] Training on {len(train_data)} samples, validating on {len(val_data)} samples"
    )
...
def _apply_oversampling(data: list[dict]) -> list[dict]:
    ...
    category_counts = Counter(
        item.get("category") for item in data if "category" in item
    )
    ...
    try:
        ...
        for run in runs[keep_count:]:
            try:
                ...
                print(
                    f"[WARN] Failed to delete run {run.info.run_id}: {e}",
                    file=sys.stderr,
                )
...
async def retrain_pipeline(
    ...
    try:
        ...
        merged_data = merge_datasets_with_overrides(
            dataset_files, overrides, override_weight=2
        )
        ...
        if equitable_recall_spread > equitable_recall_threshold:
            print(
                f"[WARN] Equitable recall spread {equitable_recall_spread} exceeds threshold {equitable_recall_threshold}"
            )
            ...
            if (
                metrics_weighted.get("equitable_recall_spread", 1.0)
                < equitable_recall_spread
            ):
                print(
                    "[INFO] Class-weighted loss improved equitable recall, using weighted model"
                )
                model_version = model_version_weighted
                metrics = metrics_weighted
            else:
                print(
                    "[INFO] Class-weighted loss did not improve, trying oversampling..."
                )
                ...
                metrics_oversampled = await evaluate_model(
                    test_data, model_version_oversampled
                )

                # Use best of the three
                if (
                    metrics_oversampled.get("equitable_recall_spread", 1.0)
                    < equitable_recall_spread
                ):
                    print(
                        "[INFO] Oversampling improved equitable recall, using oversampled model"
                    )
                    model_version = model_version_oversampled
                    metrics = metrics_oversampled
                else:
                    print(
                        "[WARN] Neither class-weighted loss nor oversampling improved equitable recall"
                    )
...
def main():
    ...
    if args.skip_download and not args.dataset_dir:
        print(
            "[ERROR] --dataset-dir required when using --skip-download", file=sys.stderr
        )
        sys.exit(1)
