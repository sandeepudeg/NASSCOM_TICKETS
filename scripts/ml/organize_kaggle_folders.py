...
def organize():
    ...
    cursor.execute("SELECT id, category FROM tickets WHERE owner_id = 'kaggle_importer'")
    ...
    for ticket_id, category in tickets:
        ...
        cursor.execute("SELECT 1 FROM ticket_folder_assignments WHERE ticket_id = ? AND folder_id = ?", (ticket_id, folder_id))
        if cursor.fetchone():
            already_assigned += 1
            continue
    ...
    conn.commit()
    conn.close()
    print(f"Organization complete! Newly assigned: {assigned_count}, Already assigned: {already_assigned}")
