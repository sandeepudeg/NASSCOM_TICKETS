...
def inspect_json_file(file_path: Path, max_samples: int = 5) -> dict:
    ...
    try:
        ...
        if category_field:
            categories = [
                item.get(category_field) for item in data if isinstance(item, dict)
            ]
...
def main():
    ...
    parser.add_argument(
        "--dataset", choices=list(DATASETS.keys()), help="Specific dataset to inspect"
    )
    parser.add_argument("--all", action="store_true", help="Inspect all three datasets")
