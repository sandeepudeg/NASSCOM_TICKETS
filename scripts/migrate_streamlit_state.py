...
def migrate_to_flask_config(preferences: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for streamlit_key, flask_key in preference_mapping.items():
        if streamlit_key in preferences:
            ...
            print(
                f"🔄 Mapped {streamlit_key} -> {flask_key}: {preferences[streamlit_key]}"
            )
...
def save_migration_report(
    state_files: List[Path], preferences: Dict[str, Any], flask_config: Dict[str, Any]
) -> None:
...
def main():
    ...
    if not state_files:
        print(
            "ℹ️  No Streamlit state files found - this is expected if Streamlit was already removed"
        )
    ...
    print("\n📄 Documentation:")
    print("   • Migration report: migration_verification_report.json")
    if flask_config:
        print("   • Flask config file: .env.migrated")

    print("\n🗑️  Intentionally Discarded:")
    print("   • Session cache (ephemeral)")
    print("   • Widget states (UI-specific)")
    print("   • File upload state (request-scoped)")
    print("   • Temporary session data (not persistent)")
