...
class TestTemplateRendering:
    ...
    def test_dashboard_template_rendering(self, client):
        ...
        with patch.object(client.application.api_client, 'get') as mock_get:
            mock_folders = [
                {"id": "1", "name": "Test Folder", "description": "Test", "ticket_count": 5},
                {"id": "2", "name": "Another Folder", "description": "Another", "ticket_count": 3}
            ]
            ...
            with patch.object(client.application.api_client, 'post') as mock_post:
                mock_classification = {
                    "classification": {
                        ...
                        "confidence": 0.85,
                        "similar_tickets": [
                            {"id": "1", "title": "Similar Ticket", "similarity_score": 0.9}
                        ]
                    }
                }
    ...
    def test_classify_test_template_with_results(self, client):
        ...
        with patch.object(client.application.api_client, 'post') as mock_post:
            ...
            response = client.post('/classify-test', data={
    ...
    def test_error_500_template_rendering(self, client):
        ...
        with patch.object(client.application.api_client, 'get') as mock_get:
            ...
            assert response.status_code in [200, 500]  # May handle gracefully or show error page
    ...
    def test_error_403_template_rendering(self, app_context):
        ...
        with app_context.test_client() as client:
            ...
            with pytest.raises(Exception):  # This will be caught by error handler
    ...
    def test_template_responsive_design(self, client):
        ...
        with patch.object(client.application.api_client, 'get') as mock_get:
            ...
            assert b'col-' in response.data or b'container' in response.data  # Bootstrap grid
    ...
    def test_template_accessibility_features(self, client):
        ...
        with patch.object(client.application.api_client, 'get') as mock_get:
            ...
            assert b'alt=' in response.data or b'aria-' in response.data or b'role=' in response.data
    ...
    def test_all_templates_render_without_errors(self, client):
        ...
        for route, mock_returns in routes_to_test:
            with patch.object(client.application.api_client, 'get') as mock_get:
                ...
                assert response.status_code in [200, 301, 302, 303, 307, 308], f"Route {route} failed with status {response.status_code}"
                
                # Should not contain Python error traces
                assert b'Traceback' not in response.data
                assert b'Exception' not in response.data
                assert b'Error:' not in response.data or b'Error loading' in response.data  # Allow user-friendly error messages