...
def test_escalation_queue_review_and_override(page: Page, base_url: str):
    ...
    original_category = first_ticket.locator('[data-testid="suggested-category"]').inner_text()
    ...
    corrected_category = next(cat for cat in valid_categories if cat != original_category)
    
    page.select_option('select[name="correctedCategory"]', corrected_category)
    
    # Optionally add override notes
    page.fill('textarea[name="overrideNotes"]', f"Correcting from {original_category} to {corrected_category}")
    
    # Submit the override
    page.click('button[data-testid="submit-override"]')
    
    # Verify success notification/toast appears
    success_toast = page.locator('[data-testid="success-toast"], .ant-message-success, [role="alert"]')
    ...
    remaining_tickets = page.locator(f'[data-testid="escalation-item"][data-ticket-id="{ticket_id}"]')
...
def test_escalation_queue_sorting_oldest_first(page: Page, base_url: str):
    ...
    if ticket_items.count() >= 2:
        ...
        first_timestamp_text = ticket_items.nth(0).locator('[data-testid="created-at"]').inner_text()
        second_timestamp_text = ticket_items.nth(1).locator('[data-testid="created-at"]').inner_text()
    ...
    if first_ticket.count() > 0:
        ...
        if similar_tickets_section.is_visible():
            ...
            if similar_ticket_items.count() > 0:
                ...
                similarity_score = first_similar.locator('[data-testid="similarity-score"]')
...
def test_escalation_override_audit_trail(page: Page, base_url: str, api_base_url: str):
    ...
    if first_ticket.count() > 0:
        ...
        valid_categories = ["Infrastructure", "Application", "Security", "Database", "Storage", "Network", "Access Management"]
        corrected_category = next(cat for cat in valid_categories if cat != original_category)
        ...
        # Wait for success
        expect(page.locator('[data-testid="success-toast"], .ant-message-success')).to_be_visible(timeout=5000)
        ...
        if audit_section.is_visible():
            ...
            override_entry = audit_section.locator('[data-testid="audit-entry"][data-action="override"]')
