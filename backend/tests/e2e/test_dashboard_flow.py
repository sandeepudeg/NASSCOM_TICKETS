...
def test_dashboard_auto_refresh_without_page_reload(page: Page, base_url: str):
    ...
    initial_open_tickets = open_tickets_card.inner_text()
    
    expect(escalation_queue_card).to_be_visible()
    initial_escalation_depth = escalation_queue_card.inner_text()
    
    expect(pattern_alerts_card).to_be_visible()
    initial_pattern_alerts = pattern_alerts_card.inner_text()
    ...
    background_page.fill('textarea[name="description"]', "This ticket is created to test dashboard refresh")
    ...
    assert marker_timestamp == marker_timestamp_after, "Page was fully reloaded (marker timestamp changed)"
    ...
    updated_open_tickets = open_tickets_card.inner_text()
...
def test_dashboard_navigation_to_detail_pages(page: Page, base_url: str):
    ...
    # Wait for dashboard to load
    expect(page.locator('[data-testid="dashboard-content"]')).to_be_visible(timeout=5000)
...
def test_dashboard_loading_states(page: Page, base_url: str):
    ...
    loading_indicator = page.locator('[data-testid="loading-spinner"], .ant-spin, [role="progressbar"]')
...
def test_dashboard_error_handling(page: Page, base_url: str):
    ...
    page.route("**/api/v1/dashboard/metrics", lambda route: route.fulfill(
        status=500,
    ...
    error_message = page.locator('[data-testid="error-message"], .ant-alert-error, [role="alert"]')
...
def test_dashboard_refresh_button(page: Page, base_url: str):
    ...
    refresh_button = page.locator('[data-testid="refresh-button"], button:has-text("Refresh")')
