...
def test_folder_create_assign_ticket_view_contents(page: Page, base_url: str):
    ...
    folder_name_input = page.locator(
        'input[name="folderName"], input[placeholder*="folder name" i]'
    )
    ...
    page.fill(
        'textarea[name="description"]', "This is a test ticket for folder assignment"
    )
    ...
    expect(page.locator('[data-testid="classification-result"]')).to_be_visible(
        timeout=15000
    )

    # Assign ticket to folder
    # Look for "Assign to Folder" button or dropdown
    assign_button = page.locator(
        '[data-testid="assign-to-folder-button"], button:has-text("Assign to Folder")'
    )
    ...
    folder_dropdown = page.locator(
        'select[name="folderId"], [data-testid="folder-select"]'
    )
    ...
    ticket_item = page.locator(
        f'[data-testid="ticket-item"][data-ticket-id="{ticket_id}"]'
    )
...
def test_folder_creation_validation(page: Page, base_url: str):
    ...
    error_message = page.locator(
        '[data-testid="error-message"], .ant-form-item-explain-error, [role="alert"]'
    )
...
def test_remove_ticket_from_folder(page: Page, base_url: str):
    ...
    remove_button = ticket_item.locator(
        '[data-testid="remove-ticket-button"], button:has-text("Remove")'
    )
    expect(remove_button).to_be_visible()
    remove_button.click()

    # Confirm removal if there's a confirmation dialog
    confirm_remove = page.locator(
        'button:has-text("Confirm"), button:has-text("Yes"), .ant-modal button.ant-btn-primary'
    )
    ...
    # Wait for success notification
    success_notification = page.locator(
        '[data-testid="success-toast"], .ant-message-success'
    )
    expect(success_notification).to_be_visible(timeout=5000)
...
def test_folder_list_displays_all_user_folders(page: Page, base_url: str):
    ...
    for folder_name in folder_names:
        ...
        folder_item = page.locator(
            f'[data-testid="folder-item"]:has-text("{folder_name}")'
        )
        expect(folder_item).to_be_visible(timeout=5000)
