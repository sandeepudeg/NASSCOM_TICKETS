...
def test_ticket_submission_to_classification_display(
    ...
    assert (
        category_text in valid_categories
    ), f"Category '{category_text}' is not in valid categories"
    ...
    confidence_value = float(confidence_text.strip("%").strip()) / (
        100 if "%" in confidence_text else 1
    )
    assert (
        0.0 <= confidence_value <= 1.0
    ), f"Confidence score {confidence_value} is not in range [0.0, 1.0]"
    ...
    assert (
        routing_status in valid_statuses
    ), f"Routing status '{routing_status}' is not valid"
...
def test_ticket_classification_with_structured_input(page: Page, base_url: str):
  ...
  "http.status_code": 502
    ...
    page.fill(
        'textarea[name="description"]', "Seeing increased 502 errors from API gateway"
    )
