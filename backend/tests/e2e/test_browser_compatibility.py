...
def test_responsive_layout_mobile(page: Page, base_url: str):
    ...
    folder_button = page.locator('button:has-text("Folders"), button[aria-label*="folder"]').first
    expect(folder_button).to_be_visible()
    
    # Verify hamburger menu button is visible (mobile only)
    menu_button = page.locator('button:has([class*="MenuOutlined"]), button[aria-label*="menu"]').first
...
def test_ticket_submission_cross_browser(page: Page, base_url: str):
    ...
    page.fill('textarea[name="description"], textarea[placeholder*="description"]', "Test description")
...
def test_folder_operations_cross_browser(page: Page, base_url: str):
    ...
    if viewport_width < 1024:
        ...
        page.locator('button:has-text("Folders"), button[aria-label*="folder"]').first.click()
    
    # Click "New Folder" button
    page.locator('button:has-text("New Folder")').first.click()
    
    # Fill in folder name
    page.fill('input[placeholder*="Folder name"]', f"Test Folder {page.context.browser.browser_type.name}")
