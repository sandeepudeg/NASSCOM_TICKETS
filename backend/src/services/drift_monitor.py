...
class DriftMonitor:
    ...
    async def check_drift(
        self,
        training_distribution: Optional[dict[str, float]] = None,
        window_hours: int = 24,
    ...
    async def send_drift_alert(self, drift_details: list[dict]):
        ...
        if not settings.observability_webhook_url:
            print("[WARN] No observability webhook URL configured, skipping drift alert")
            return
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code >= 200 and response.status_code < 300:
                    print(f"[INFO] Drift alert sent successfully: {response.status_code}")
                else:
    ...
    async def monitor_and_alert(
        ...
        if has_drift:
            print(f"[ALERT] Category distribution drift detected:")
...
async def load_training_distribution_from_mlflow() -> Optional[dict[str, float]]:
    """
