...
def check():
    ...
    for row in stats:
        print(f"Status: {row[0]}, Total: {row[1]}, Has Resolution: {row[2]}, Has Confidence: {row[3]}")
    ...
    cursor.execute("SELECT id, title, status, resolution_steps_json FROM tickets WHERE status='resolved' LIMIT 1")
