...
class Config:
    ...
    def update_config(self):
        ...
        self.ENABLE_STATIC_ASSET_VERSIONING = True  # Enable asset versioning for cache busting
        ...
        self.CACHE_HEALTH_TIMEOUT = 60    # 1 minute for health checks (real-time monitoring)
        self.CACHE_ESCALATIONS_TIMEOUT = 180  # 3 minutes for escalations (moderate frequency)
        self.CACHE_CLASSIFICATION_TIMEOUT = 600  # 10 minutes for classification results (expensive operations)
...
def is_hf_spaces():
    ...
    if port == '7860' and host == '0.0.0.0':
        if hostname and ('hf.space' in hostname or 'huggingface' in hostname.lower() or 'space' in hostname.lower()):
    ...
    if all(os.path.exists(path) for path in hf_paths):
        ...
        if hostname and (len(hostname) > 10 or 'space' in hostname.lower()):
            print(f"🎯 HF Spaces detected via container structure and hostname: {hostname}")
            return True
...
def configure_for_hf_spaces():
    ...
    if is_hf_spaces():
        ...
        print(f"⚙️  HF Spaces configuration complete:")
        ...
        print(f"   - Default PORT: {os.getenv('PORT', '5000')} (local), 7860 (HF Spaces)")
        print(f"   - Default CORS: http://localhost:5001")